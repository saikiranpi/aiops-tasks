...
resource "aws_security_group" "gitlab_sg" {
  name        = "gitlab_sg"
  description = "Allow inbound traffic for self-hosted GitLab"
...
resource "aws_instance" "gitlab_server" {
  ...
  instance_type = "t3.medium" # t3.medium has 2 vCPUs and 4GB RAM
  key_name      = "rv-usa"    # Ensure this key pair exists in your us-east-1 region or update it
  ...
  user_data = <<-EOF
              ...
              # Get the instance's public IP address using IMDSv2
